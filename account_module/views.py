    ...
    def post(self, request):
        ...
        if register_form.is_valid():
            ...
            if user:
                ...
                messages.success(
                    request,
                    "لینک فعالسازی به ایمیل شما ارسال شد. لطفاً ایمیل خود را بررسی کنید.",
                )
        ...
        if login_form.is_valid():
            ...
            if user is not None:
                if not user.is_active:
                    ...
                    return render(
                        request, "account_module/login.html", {"login_form": login_form}
                    )
...
class activate_accountView(View):
    def get(self, request, email_activate_code):
        user: User = User.objects.filter(
            email_active_code__iexact=email_activate_code
        ).first()
        if user is not None:
            if not user.is_active:
                user.is_active = True
...
class forgot_passwordView(View):
    def get(self, request):
        ...
        return render(
            request,
            "account_module/forgot_password.html",
...
class forgot_passwordConfirmView(View):
    def get(self, request, confirmation_code):
        user: User = User.objects.filter(
            email_active_code__iexact=confirmation_code
        ).first()
        if user is None:
            return redirect(reverse("login"))

        reset_password_context = reset_password()
        context = {"reset_password_context": reset_password_context, "user": user}
