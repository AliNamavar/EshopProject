...
class ArticleDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["comment"] = (
            ArticleComments.objects.filter(article_id=article.id, parent=None)
            .prefetch_related("articlecomments_set")
            .order_by("-created_date")
        )
        context["comments_count"] = ArticleComments.objects.filter(
            article_id=article.id
        ).count()
        context["banners"] = site_banner.objects.filter(
            is_active=True,
            position__iexact=site_banner.site_banner_position.article_detail,
        )
        return context


def article_category_component(request):
    main_categorys = ArticleCategory.objects.filter(
        is_active=True, parent_id=None
    ).prefetch_related("articlecategory_set")
    context = {"main_categorys": main_categorys}
...
def article_comments(request):
    if request.user.is_authenticated:
        ...
        new_article_comment = ArticleComments(
        ...
        context = {
            "comment": ArticleComments.objects.filter(
                article_id=article_id, parent=None
            )
            .prefetch_related("articlecomments_set")
            .order_by("-created_date"),
            "comments_count": ArticleComments.objects.filter(
                article_id=article_id
            ).count(),
        }
