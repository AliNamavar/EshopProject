    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.user.is_authenticated:
            if int(count) < 1:
                ...
                product_get = product.objects.filter(
                ...
                if product_get is None:
                    ...
                    current_order, created = Order_Model.objects.get_or_create(
                        User=request.user, is_paid=False
                    )
                    product_detail = current_order.order_detail_set.filter(
                        product_id=product_get
                    ).first()
...
class Order_View(View):
    def get(self, request, *args, **kwargs):
        current_order, created = Order_Model.objects.prefetch_related(
            "order_detail_set"
        ).get_or_create(User=request.user, is_paid=False)
...
def Order_remove_product(request):
    ...
    if product_id is not None:
        ...
        product_detail = Order_detail.objects.filter(
            product_id=product_id, order=current_order
        ).first()
        if product_detail is None:
            return JsonResponse(
                {"status": "error", "message": "محصول مورد نظر یافت نشد toye"}
            )
        else:
            ...
            return JsonResponse(
                {
                    ...
                    "data": render_to_string(
                        "order_partials/order_rm_partials.html",
                        context={
                            "Order": current_order,
                            "total_price": total_price,
                        },
...
class EditOrderDetailCount(View):
    def post(self, request, *args, **kwargs):
        ...
        if Detail is None:
            return JsonResponse(
                {"status": "error", "message": "محصول مورد نظر سافت نشد"}
            )
