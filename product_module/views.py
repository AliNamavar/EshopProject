...
class ProductListView(ListView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        context["banners"] = site_banner.objects.filter(
            is_active=True,
            position__iexact=site_banner.site_banner_position.product_list,
        )
        return context
...
class ProductDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["banners"] = site_banner.objects.filter(
            is_active=True,
            position__iexact=site_banner.site_banner_position.product_detail,
        )
...
def products_category_component(request):
    products_category = productCategory.objects.filter(
        is_active=True, parent_id=None, is_deleted=False
    ).prefetch_related("productcategory_set")
    context = {"products_category": products_category}
    return render(
        request, "Category_component/product_category_component.html", context
    )


def product_brand_component(request):
    product_brand = product_Brands.objects.filter(is_active=True).annotate(
        product_count=Count("product")
    )
    context = {"product_brand": product_brand}
