...
class ChangePasswordForm(forms.Form):
    correct_password = forms.CharField(
        widget=forms.PasswordInput(attrs={"class": "form-control"}), label="پسوورد فعلی"
    )
    new_password = forms.CharField(
        widget=forms.PasswordInput(attrs={"class": "form-control"}), label="پسوورد جدید"
    )
    ...
    def clean(self):
        ...
        if new_password != correct_password:
            self.add_error(
                "Repeat_new_password", "پسوورد جدید و تکرار آن مطابقت ندارند."
            )
        return cleaned_data
