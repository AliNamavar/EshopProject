...
class editUserProfile(View):
    ...
    def get(self, request):
        ...
        context = {"form": edit_form, "correct_user": correct_user}
        return render(request, "user_panel_module/edit_profile_page.html", context)

    def post(self, request):
        correct_user = User.objects.filter(id=request.user.id).first()
        edit_form = EditProfileModelForm(
            request.POST, request.FILES, instance=correct_user
        )
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            if current_user.check_password(form.cleaned_data.get("correct_password")):
                ...
                form.add_error("correct_password", "کلمه ی عبور وارد شده اشتباه میباشد")
